...
function showInvoiceRow($seq_no, $row)
{
  ...
  <tr>
    ...
    <td><?php echo isset($row['INVOICE_DATE']) ? $row['INVOICE_DATE'] : ''; ?></td>
    <td><?php echo isset($row['TOTAL_AMOUNT']) ? $row['TOTAL_AMOUNT'] : ''; ?></td>
    <td><?php echo isset($row['TOTAL_DISCOUNT']) ? $row['TOTAL_DISCOUNT'] : ''; ?></td>
    <td><?php echo isset($row['NET_TOTAL']) ? $row['NET_TOTAL'] : ''; ?></td>
    <td>
      <button class="btn btn-warning btn-sm"
          onclick="printInvoice(<?php echo isset($row['INVOICE_ID']) ? $row['INVOICE_ID'] : ''; ?>);">
        <i class="fa fa-fax"></i>
      </button>
      <button class="btn btn-danger btn-sm"
          onclick="deleteInvoice(<?php echo isset($row['INVOICE_ID']) ? $row['INVOICE_ID'] : ''; ?>);">
...
function printInvoice($invoice_number)
{
  ...
  if ($con) {
    ...
    $query = "SELECT * FROM sales INNER JOIN customers ON customers.ID = sales.ID WHERE INVOICE_NUMBER = $invoice_number";
    $result = mysqli_query($con, $query);
    $row = mysqli_fetch_array($result);
    $customer_name = isset($row['NAME']) ? $row['NAME'] : '';
    $address = isset($row['ADDRESS']) ? $row['ADDRESS'] : '';
    $contact_number = isset($row['CONTACT_NUMBER']) ? $row['CONTACT_NUMBER'] : '';
    $doctor_name = isset($row['DOCTOR_NAME']) ? $row['DOCTOR_NAME'] : '';
    $doctor_address = isset($row['DOCTOR_ADDRESS']) ? $row['DOCTOR_ADDRESS'] : '';
    ...
    $total_discount = isset($row['TOTAL_DISCOUNT']) ? $row['TOTAL_DISCOUNT'] : '';
    $net_total = isset($row['NET_TOTAL']) ? $row['NET_TOTAL'] : '';
  }
  ...
  <div class="row">
    ...
    <div class="col-md-10 h3" style="color: #ff5252;">Customer Invoice<span
          class="float-right">Invoice Number : <?php echo $invoice_number; ?></span></div>
  </div>
  ...
  <div class="row">
    ...
    $result = mysqli_query($con, $query);
    $row = mysqli_fetch_array($result);
    $p_name = isset($row['PHARMACY_NAME']) ? $row['PHARMACY_NAME'] : '';
    $p_address = isset($row['ADDRESS']) ? $row['ADDRESS'] : '';
    $p_email = isset($row['EMAIL']) ? $row['EMAIL'] : '';
    $p_contact_number = isset($row['CONTACT_NUMBER']) ? $row['CONTACT_NUMBER'] : '';
    ?>
    ...
    <div class="col-md-10 table-responsive">
      <table class="table table-bordered table-striped table-hover" id="purchase_report_div">
        ...
        while ($row = mysqli_fetch_array($result)) {
          ...
          <tr>
            <td><?php echo $seq_no; ?></td>
            <td><?php echo isset($row['MEDICINE_NAME']) ? $row['MEDICINE_NAME'] : ''; ?></td>
            <td><?php echo isset($row['EXPIRY_DATE']) ? $row['EXPIRY_DATE'] : ''; ?></td>
            <td><?php echo isset($row['QUANTITY']) ? $row['QUANTITY'] : ''; ?></td>
            <td><?php echo isset($row['MRP']) ? $row['MRP'] : ''; ?></td>
            <td><?php echo isset($row['DISCOUNT']) ? $row['DISCOUNT'] . "%" : ''; ?></td>
